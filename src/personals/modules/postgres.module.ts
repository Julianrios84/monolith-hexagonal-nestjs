...
import { DataPersonal } from '@personals/infrastructure/entities/postgres';
import { PostgresPersonalRepository } from '@personals/infrastructure/repository';
import { PersonalController } from '@personals/infrastructure/personal.controller';

@Module({
  controllers: [PersonalController],
  imports: [
    CONFIG_DATABASE_POSTGRES_ENTITY(DataPersonal, Connection.personal.name),
  ],
  providers: [
    PersonalService,
    {
      provide: IRepository,
      useClass: PostgresPersonalRepository,
    },
    {
