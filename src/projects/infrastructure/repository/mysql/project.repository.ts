...
@Injectable()
export class MySQLProjectRepository implements IRepository {
  constructor(
  ...
  async create(user_id: string, body: ICreateDto): Promise<IGetDto> {
    const record = await this.repository.save({ project_id: await this.uuidService.create(), user_id,  ...body })
