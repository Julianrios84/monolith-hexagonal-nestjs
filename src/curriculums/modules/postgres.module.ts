...
import { Curriculum } from '@curriculums/infrastructure/entities/postgres';
import { PostgresCurriculumRepository } from '@curriculums/infrastructure/repository';
import { CurriculumController } from '@curriculums/infrastructure/curriculum.controller';

@Module({
  controllers: [CurriculumController],
  imports: [
    CONFIG_DATABASE_POSTGRES_ENTITY(Curriculum, Connection.curriculum.name),
  ],
  providers: [
    CurriculumService,
    {
      provide: IRepository,
      useClass: PostgresCurriculumRepository,
    },
    {
