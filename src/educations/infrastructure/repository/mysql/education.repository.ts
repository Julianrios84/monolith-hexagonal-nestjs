...
import { Education } from "@educations/infrastructure/entities/mysql";
...
@Injectable()
export class MySQLEducationRepository implements IRepository {
  constructor(
  ...
  async create(user_id: string, body: ICreateDto): Promise<IGetDto> {
    const record = await this.repository.save({ education_id: await this.uuidService.create(), user_id,  ...body })
  ...
  async delete(user_id: string, id: string): Promise<IDeleteDto> {
    await this.findOne(user_id, id);
    await this.repository.delete({ education_id: id, user_id });
