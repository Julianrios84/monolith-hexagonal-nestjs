...
@Injectable()
export class MySQLCertificationRepository implements IRepository {
  constructor(
  ...
  async create(user_id: string, body: ICreateDto): Promise<IGetDto> {
    const record = await this.repository.save({ certification_id: await this.uuidService.create(), user_id,  ...body })
